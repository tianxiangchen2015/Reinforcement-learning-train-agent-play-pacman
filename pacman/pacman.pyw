...
#        self.fruitScorePos = (0, 0)
        ...
        self.screenfTilePos = (0, 0) # nearest-tile position of the screen from the UL corner
...
class pacman ():
    ...
    def __init__ (self):
        ...
        self.net = buildNetwork(INPUTS, HIDDEN1, HIDDEN2, OUTPUTS, hiddenclass=LinearLayer, outclass=LinearLayer, recurrent=True, bias=True) 
    ...
    def Move (self):
        ...
        self.nearestRow = int(((self.y + 8) / 16))
        self.nearestCol = int(((self.x + 8) / 16))
        #print self.nearestRow #self.nearestCol
    ...
    def FollowNextPathWay (self):
        if self.dots:
            curS_a = [self.x, self.y, ghosts[0].x, ghosts[0].y, ghosts[0].state, ghosts[1].x, ghosts[1].y, ghosts[1].state, ghosts[2].x, ghosts[2].y, ghosts[2].state, ghosts[3].x, ghosts[3].y, ghosts[3].state]
            q = range(0,len(self.dots),1)
            for k in range(0,len(self.dots), 1):
                if k > 0 :
                    curS_a.pop()
                curS_a.append(self.dots[k])
                q[k] = net.activate(curS_a)
            n_mq = max(q)
            x = q.index(max(q))
            action = x
            ...
            if epsilon < self.Epsilon:
                while action == x:
                    action = random.randint(0,len(self.dots)-1)            
            self.dots = []
            mq = q[action]        
            mq = mq + alpha * (self.reward + gamma * n_mq - mq)
            ds.newSequence()
            ds.clear()
            ds.appendLinked(tuple(curS_a),tuple(mq))
        ...
        if not self.currentPath == False:
            ...
            if len(self.currentPath) > 0:
                ...
                randRow = dot[action]/21
                randCol = dot[action]%21
                
                self.currentPath = path.FindPath( (self.nearestRow, self.nearestCol), (randRow, randCol) )
                self.FollowNextPathWay()     

#     next_State=[self.x, self.y, ghosts[0].x, ghosts[0].y, ghosts[0].state, ghosts[1].x, ghosts[1].y, ghosts[1].state, ghosts[2].x, ghosts[2].y, ghosts[2].state, ghosts[3].x, ghosts[3].y, ghosts[3].state]
#        curS_a = next_State
#        q = range(0,len(dot),1)
#        for k in range(0,len(dot), 1):
#            if k > 0 :
#                curS_a.pop()
#            curS_a.append(dot[k])
#            q[k] = net.activate(curS_a)
...
#        if epsilon < Epsilon:
#            while action == x:
#                action = random.randint(0,len(dot)-1)
#
#        player.targetX = dot[action]/21
#        player.targetY = dot[action]%21
##        print [player.targetX,player.targetY]
#        
#        dot = []
#        # reward update q-function
#        mq = q[action]        
#        mq = mq + alpha * (reward + gamma * n_mq - mq)
#        
...
if not os.path.exists('PacmanPathNN'):
    print('create NN')
    net = buildNetwork(INPUTS, HIDDEN1, HIDDEN2, OUTPUTS, hiddenclass=LinearLayer, outclass=LinearLayer, recurrent=True, bias=True) 
    ds = SequentialDataSet(INPUTS, OUTPUTS)
    net.randomize()
    Epsilon = 0.300
else:
    print('open file')
    net = NetworkReader.readFrom('PacmanPathNN')
    ds = SequentialDataSet(INPUTS, OUTPUTS)
    f = open('pathexplore', 'r')
    temp = f.read().splitlines()
    Epsilon = float(str(temp[0]))
...
while True: 
   ...
   # curState = [player.x, player.y player.velX, player.velY]
    #, ghosts[0].x, ghosts[0].y, ghosts[1].x, ghosts[1].y, ghosts[2].x, ghosts[2].y, ghosts[3].x, ghosts[3].y]
    
    
    if thisGame.mode == 1:
        firstTime = 0
       
        curState=[player.x, player.y, ghosts[0].x, ghosts[0].y, ghosts[0].state, ghosts[1].x, ghosts[1].y, ghosts[1].state, ghosts[2].x, ghosts[2].y, ghosts[2].state, ghosts[3].x, ghosts[3].y, ghosts[3].state]

        stemp = thisGame.score
        ...
        if thisGame.score - stemp == 10:
            reward = reward + 1
        elif thisGame.score - stemp == 100:
            reward = reward + 1
        elif thisGame.score - stemp == 200:
            reward = reward + 1.3
        elif thisGame.score - stemp == 400:
            reward = reward + 1.6
        elif thisGame.score - stemp == 800:
            reward = reward + 1.9
        elif thisGame.score - stemp == 1600:
            reward = reward + 2.2
        else :
            reward = reward - 0.0001 * dotsnum
            
    elif thisGame.mode == 2:
        # waiting after getting hit by a ghost
        thisGame.modeTimer += 1        
        reward = reward - 5
        curS_a = [player.x, player.y, ghosts[0].x, ghosts[0].y, ghosts[0].state, ghosts[1].x, ghosts[1].y, ghosts[1].state, ghosts[2].x, ghosts[2].y, ghosts[2].state, ghosts[3].x, ghosts[3].y, ghosts[3].state]
        q = range(0,len(dot),1)
        for k in range(0,len(dot), 1):
            if k > 0 :
                curS_a.pop()
        ...
        ds.clear()
        ds.appendLinked(tuple(curS_a),tuple(mq))
        player.trainer = BackpropTrainer(player.net, ds)
        ...
        f = open('PathMethodScore', 'a')        
        f.write(str(thisGame.score))
        f.write('\n')
        f = open('PathMethodLeftBeans', 'a')        
        f.write(str(196 - dotsnum))
        f.write('\n')
        f.close()
        dotsnum = 196
        saveiter = saveiter + 1
        ...
        thisGame.modeTimer += 1
        #reward = reward + thisGame.score - stemp
    ...
    if firstTime == 0:
        ii = 0
        dot = range(0,dotsnum,1)
        for k in range(0,len(thisLevel.map), 1):
            if thisLevel.map[k] == 2 or thisLevel.map[k] == 3:
                dot[ii] = k
                ii = ii + 1
        dotsnum = ii + 1
        player.dots = dot
...
#        player.net = net
    ...
    #    print len(net)
        if saveiter % 2 == 0:
            saveiter = 1
            if os.path.exists('PacmanPathNN'):
                os.remove('PacmanPathNN')
            NetworkWriter.writeToFile(net, 'PacmanPathNN')
            f = open('pathexplore', 'w')
            f.write(str(Epsilon))
            f.close()
